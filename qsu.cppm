...
export class main {
  ...
  void fill(auto &q, const sprite::compo &set) {
    q->map_colours([&](auto *cs) {
      for (auto _ : set) {
        *cs++ = {};
      }
    });
    q->map_multipliers([&](auto *ms) {
      for (auto [spr, _] : set) {
        *ms++ = {1, 1, 1, 1};
      }
    });
    q->map_positions([&](auto *ps) {
      for (auto [spr, _] : set) {
...
public:
  ...
  void fill_sprites(const sprite::compo &set) { fill(m_spr, set); }
};
} // namespace qsu
